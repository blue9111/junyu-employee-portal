...
const taskTemplates:Array<{title:string;priority:Task['priority']}>=[{title:'審查待處理文件',priority:'high'},{title:'準備工作簡報',priority:'normal'},{title:'更新工作文件',priority:'low'}];
...
export default function App(){
  ...
  function openTaskTemplate(title:string,priority:Task['priority']){setForm({...emptyTask(),title,priority});setModal(true);}
  ...
  return <div className="portal-shell">
    ...
    <main className="portal-main"><div className="page-content">
      ...
      <section className="dashboard-grid">{cardOrder.map(id=>{
        ...
        if(id==='tasks')return <article {...cardProps(id)}><div className="card-title task-card-title"><span className="icon-box green"><Check/></span><h3>我的任務</h3>{dragButton}</div>{visible.length||(!query.trim()&&tasks.length<3)?<div className={`compact-tasks ${tasks.length?'':'task-templates'}`}>{visible.slice(0,3).map(task=><button key={task.id} className={`compact-task ${task.status}`} disabled={busy} onClick={()=>void advance(task)}><span className="task-check">{task.status==='done'?<Check/>:null}</span><b>{task.title}</b><i className={task.priority}/></button>)}{!query.trim()&&visible.length<3&&taskTemplates.slice(0,3-visible.length).map(item=><button key={item.title} className="compact-task task-template" onClick={()=>openTaskTemplate(item.title,item.priority)}><span className="task-check"/><b>{item.title}</b><i className={item.priority}/></button>)}<small>{tasks.length?`${counts.done}/${tasks.length} 已完成`:'點選範例即可建立第一項工作'}</small></div>:<div className="task-no-result">沒有符合搜尋條件的任務</div>}</article>;

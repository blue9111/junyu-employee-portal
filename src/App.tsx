  ...
  return <div className="portal-shell">
    ...
    <main className="portal-main"><div className="page-content">
      ...
      <section className="dashboard-grid">{cardOrder.map(id=>{
        ...
        if(id==='tasks')return <article {...cardProps(id)}><div className="card-title task-card-title"><span className="icon-box green"><Check/></span><h3>我的任務</h3>{dragButton}</div>{visible.length?<div className="compact-tasks">{visible.slice(0,3).map(task=><button key={task.id} className={`compact-task ${task.status}`} disabled={busy} onClick={()=>void advance(task)}><span className="task-check">{task.status==='done'?<Check/>:null}</span><b>{task.title}</b><i className={task.priority}/></button>)}<small>{counts.done}/{tasks.length} 已完成</small></div>:tasks.length?<div className="task-no-result">沒有符合搜尋條件的任務</div>:<div className="compact-tasks task-templates">{taskTemplates.map(item=><button key={item.title} className="compact-task" onClick={()=>openTaskTemplate(item.title,item.priority)}><span className="task-check"/><b>{item.title}</b><i className={item.priority}/></button>)}<small>點選範例即可建立第一項工作</small></div>}</article>;

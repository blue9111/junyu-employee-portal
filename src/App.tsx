...
export default function App(){
  ...
  const points=counts.done*50+counts.doing*15+dashboard.checkins*5;
  const level=Math.max(1,Math.floor(points/250)+1);
  const badges=[tasks.length>0,counts.done>=1,completion===100&&tasks.length>0,dashboard.checkins>0].filter(Boolean).length;
  ...
  async function sendMessage(event:FormEvent){event.preventDefault();if(!chatText.trim())return;setBusy(true);try{setDashboard(await api('/api/dashboard/chat','POST',{text:chatText}));setChatText('');}catch(error){setMessage(error instanceof Error?error.message:'訊息傳送失敗。');}finally{setBusy(false);}}
  ...
  return <div className="portal-shell">
    ...
    <main className="portal-main"><div className="page-content">
      ...
      <section className="dashboard-grid">{cardOrder.map(id=>{
        if(id==='stats')return <article {...cardProps(id)}><div className="card-title"><span className="icon-box blue"><BriefcaseBusiness/></span><h3>我的統計</h3>{dragButton}</div><div className="mini-stats"><div className="blue"><BriefcaseBusiness/><strong>{tasks.length}</strong><span>全部工作</span></div><div className="amber"><Clock3/><strong>{counts.doing}</strong><span>進行中</span></div><div className="green"><Check/><strong>{completion}%</strong><span>完成率</span></div></div></article>;
      ...
      <section className="social-grid"><article className="panel chat-panel"><div className="section-title"><span className="icon-box blue"><MessageCircle/></span><div><h3>員工聊天室</h3><p>#general · 公司帳號限定</p></div></div><div className="chat-list">{dashboard.chats.length?dashboard.chats.slice(-8).map(item=><div key={item.id} className={item.authorEmail===employee.email?'mine':''}><span>{item.authorName.slice(0,1)}</span><p><b>{item.authorName}<small>{timeLabel(item.createdAt)}</small></b>{item.text}</p></div>):<div className="section-empty">目前尚無訊息，成為第一個留言的人。</div>}</div><form className="chat-form" onSubmit={sendMessage}><input maxLength={300} value={chatText} onChange={e=>setChatText(e.target.value)} placeholder="輸入訊息…" aria-label="聊天室訊息"/><button disabled={busy||!chatText.trim()}><Send/></button></form></article><article className="panel achievement-panel"><div className="section-title"><span className="icon-box violet"><Trophy/></span><div><h3>工作成就</h3><p>完成任務與互動可累積成就</p></div></div><div className="level-card"><div><span>等級</span><strong>{level}</strong></div><div><span>積分</span><strong>{points}</strong></div><div><span>徽章</span><strong>{badges}</strong></div></div><div className="achievement-list"><div className={tasks.length?'unlocked':''}><span>🎉</span><p><b>開始行動</b><small>建立第一項工作</small></p><Check/></div><div className={counts.done?'unlocked':''}><span>🏆</span><p><b>任務達人</b><small>完成第一項工作</small></p><Check/></div><div className={dashboard.checkins?'unlocked':''}><span>💓</span><p><b>團隊夥伴</b><small>完成每日心情報到</small></p><Check/></div></div></article></section>

...
type CardId='stats'|'schedule'|'tasks'|'quick'|'profile'|'news';
const defaultCardOrder:CardId[]=['stats','schedule','tasks','quick','profile','news'];
...
export default function App(){
  ...
  useEffect(()=>{if(!employee)return;const saved=localStorage.getItem(`junyu-card-order:${employee.email}`);if(!saved)return;try{const order=JSON.parse(saved) as CardId[];if(order.length===defaultCardOrder.length&&defaultCardOrder.every(id=>order.includes(id)))setCardOrder(order);}catch{/* ignore invalid saved layout */}},[employee]);
  ...
  return <div className="portal-shell">
    ...
    <main className="portal-main"><div className="page-content">
      ...
      <section className="dashboard-grid">
        {cardOrder.map(id=>{
          const props={key:id,draggable:true,onDragStart:(event:DragEvent<HTMLElement>)=>startCardDrag(event,id),onDragOver:(event:DragEvent<HTMLElement>)=>{event.preventDefault();event.dataTransfer.dropEffect='move';},onDrop:(event:DragEvent<HTMLElement>)=>dropCard(event,id),onDragEnd:()=>setDraggingCard(null),title:'拖曳卡片調整順序',className:`panel dashboard-card ${id}-card ${draggingCard===id?'is-dragging':''}`};
          if(id==='stats')return <article {...props}><div className="card-title"><span className="icon-box blue"><BriefcaseBusiness/></span><h3>我的統計</h3><GripVertical className="drag-handle"/></div><div className="mini-stats"><div className="blue"><BriefcaseBusiness/><strong>{tasks.length}</strong><span>全部工作</span></div><div className="amber"><Clock3/><strong>{counts.doing}</strong><span>進行中</span></div><div className="green"><Check/><strong>{completion}%</strong><span>完成率</span></div></div></article>;
          if(id==='schedule')return <article {...props}><div className="card-title"><span className="icon-box amber"><CalendarDays/></span><h3>今日行程</h3><small>會議服務維護中</small><GripVertical className="drag-handle"/></div><div className="schedule-list"><div className="blue"><b>工作規劃</b><span>09:00</span></div><div className="amber"><b>進度確認</b><span>14:00</span></div><div className="rose"><b>每日整理</b><span>16:30</span></div></div></article>;
          if(id==='tasks')return <article {...props}><div className="card-title"><span className="icon-box green"><Check/></span><h3>我的任務</h3><button onClick={()=>setModal(true)}><Plus/></button><GripVertical className="drag-handle"/></div>{visible.length?<div className="compact-tasks">{visible.slice(0,3).map(task=><button key={task.id} className={`compact-task ${task.status}`} disabled={busy} onClick={()=>void advance(task)}><span className="task-check">{task.status==='done'?<Check/>:task.status==='doing'?<Clock3/>:null}</span><b>{task.title}</b><i className={task.priority}/></button>)}<small>{counts.done}/{tasks.length} 已完成</small></div>:<button className="empty-compact" onClick={()=>setModal(true)}><Plus/><b>建立第一項工作</b><span>工作會與您的帳號同步</span></button>}</article>;
          if(id==='quick')return <article {...props}><div className="card-title"><span className="icon-box violet"><Sparkles/></span><h3>快速操作</h3><GripVertical className="drag-handle"/></div><div className="quick-list"><button onClick={()=>setModal(true)}><span className="blue"><Plus/></span><b>建立工作<small>新增個人待辦事項</small></b></button><button disabled><span className="amber"><CalendarDays/></span><b>會議預約<small>服務目前維護中</small></b></button>{employee.email==='jet@gotofunapp.com'&&<a href="https://takeway-company.ai.studio/"><span className="green"><Settings/></span><b>人員管理<small>開啟公司名冊後台</small></b></a>}</div></article>;
          if(id==='profile')return <article {...props}><div className="card-title"><span className="icon-box cyan"><Users/></span><h3>我的資料</h3><GripVertical className="drag-handle"/></div><div className="profile-hero"><div className="avatar large">{employee.name.slice(0,1)}</div><div><b>{employee.name}</b><span>{employee.unit||'君宇集團'} · {employee.jobTitle||'員工'}</span></div></div><dl><div><dt>工號</dt><dd>{employee.employeeId||'未設定'}</dd></div><div><dt>部門代碼</dt><dd>{employee.departmentCode||'未設定'}</dd></div><div><dt>Google 帳號</dt><dd>{employee.email}</dd></div></dl></article>;
          return <article {...props}><div className="card-title"><span className="icon-box rose"><Building2/></span><h3>公司服務</h3><GripVertical className="drag-handle"/></div><div className="news-list"><div><span/><b>員工工作入口已啟用</b><small>個人名冊與任務已同步</small></div><div><span/><b>會議預約系統</b><small>連結維護完成後開放</small></div><div><span/><b>Nexus 工作台</b><small>{message}</small></div></div></article>;
        })}
      </section>
